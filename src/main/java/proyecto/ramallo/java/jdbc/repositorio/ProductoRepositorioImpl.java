...
public class ProductoRepositorioImpl implements Repositorio<Producto>{
    //creamos la conexion y la cerramosen cada metodo(no udamos singleton)
    //cerramos la conexión en cada metodo, entonces no es necesario cerrarla en el main
    ...
    public Producto porID(Long id) {
        ...
        try(Connection conn = getConnection();
            PreparedStatement Pstmt = conn.prepareStatement("SELECT p.*,c.id_catg, c.nombre as categoria from productos as p " +
    ...
    public void eliminar(Long id) throws SQLException {
        try(Connection conn = getConnection();
            PreparedStatement pstmt = conn.prepareStatement("DELETE FROM productos WHERE id_prod=?")){
            pstmt.setLong(1,id);
            pstmt.executeUpdate();
        }
